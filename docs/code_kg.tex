...
A snippet, as used throughout this paper, is a source-grounded code excerpt
extracted at a verified file path and exact line span, annotated with node
metadata, and suitable for LLM consumption.
...
\texttt{SentenceTransformerEmbedder}, uses \texttt{all-MiniLM-L6-v2}~\cite{wang2020minilm} from the
\texttt{sentence-transformers} library~\cite{reimers2019sbert} (384-dimensional embeddings).
...
contains no probabilistic components. The graph cannot hallucinate.
...
design optimises for questions like ``What are the main themes in this document
...
We term this \emph{Structural KG-RAG}: knowledge-graph-retrieval-augmented generation, in which
the graph is extracted from formal syntax rather than inferred from natural language,
and in which every piece of retrieved context has deterministic provenance traceable
to the source. Table~\ref{tab:comparison} summarises the key distinctions.
...
\newblock In \emph{Advances in Neural Information Processing Systems
  (NeurIPS)}, 2020.
\newblock \url{https://arxiv.org/abs/2002.10957}
