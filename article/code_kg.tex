...
\title{CodeKG: Deterministic Code Navigation via Knowledge Graphs,
Semantic Indexing, and Grounded Context Packing}

\author{Eric G. Suchanek, PhD\\
Flux-Frontiers, Liberty Township OH\\[4pt]
\small\url{https://github.com/Flux-Frontiers/code\_kg}}
\date{}
...
using static analysis. Structural relationships---definitions, calls, imports,
and inheritance---and data-flow relationships---variable reads, writes, and attribute
accesses---are extracted directly from the Python abstract syntax tree and stored
...
extraction, and downstream reasoning grounded against the source.

CodeKG is independently useful without a language model: the built-in graph
explorer and hybrid query tools provide direct structural navigation and
visualization of any Python codebase. When coupled with a downstream language
model for synthesis, CodeKG additionally constitutes a form of
...
the knowledge graph is \emph{derived} from formal syntax rather than \emph{inferred}
...
graph from unstructured text. In CodeKG, the LLM has no role in graph construction:
...
The implementation is organized into four layers plus a companion data-flow
...
(emitted by \texttt{CodeKGVisitor}; see Section~\ref{sec:visitor}).

\subsection{Data-Flow Visitor (\texttt{visitor.py})}
\label{sec:visitor}

\texttt{CodeKGVisitor} extends \texttt{ast.NodeVisitor} to capture intra-function
data-flow relationships that the structural pass in \texttt{codekg.py} does not
record. Where \texttt{extract\_repo} answers \emph{what exists and how it is
called}, \texttt{CodeKGVisitor} answers \emph{what each function reads and writes}.
...
\texttt{SentenceTransformerEmbedder}, uses \texttt{all-MiniLM-L6-v2}
...
\item \texttt{build\_graph(wipe=False)} --- AST extraction, SQLite persistence,
and symbol resolution; \texttt{resolve\_symbols()} is called automatically.
...
to \texttt{GraphStore.callers\_of()}; returns all callers including cross-module
callers resolved through \texttt{sym:} stubs.
...
\textbf{Data-flow edges} are extracted by \texttt{CodeKGVisitor}: \texttt{READS}
...
two deterministic AST passes over all \texttt{.py} files. Pass~1 extracts the
definition graph. Pass~2 extracts the call graph with evidence. The resulting
nodes and edges are persisted to SQLite via \texttt{GraphStore.write()}.
A post-build step, \texttt{Graph\-Store.resolve\_symbols()}, then name-matches
every \texttt{sym:} stub against first-party definitions and writes
\texttt{RESOLVES\_TO} edges (see Section~\ref{sec:fanin}). This phase uses no
embeddings and no language models.
...
Results are deduplicated and returned as node dicts. \texttt{CodeKG.callers()}
...
exposing five tools to any MCP-compatible AI agent:
...
\item \texttt{callers(node\_id, rel)} --- fan-in lookup returning all callers of a
given node, including cross-module callers resolved through \texttt{sym:} stubs.
\end{itemize}
...
CodeKG and GraphRAG share a surface resemblance---both augment retrieval with a
...
In CodeKG, the knowledge graph is \emph{derived} from the Python abstract syntax
...
\item In CodeKG, the LLM \emph{consumes} the knowledge graph. The graph is a
...
mechanism to detect that an extracted relationship is wrong. In CodeKG, every
...
LLM and approximate. CodeKG \texttt{CALLS} edges carry machine-verifiable evidence:
...
CodeKG targets \emph{structural navigation} of a formal, syntax-governed artifact.
...
When CodeKG is coupled with a downstream language model for synthesis, the combined
...
\begin{table}[h]
\centering
\caption{GraphRAG vs.\ CodeKG (Structural KG-RAG)}
\label{tab:comparison}
\begin{tabular}{lll}
\toprule
Property & GraphRAG~\cite{edge2024graphrag} & CodeKG \\
\midrule
...
model in the loop for graph construction. Static analysis produces a precise
structural record; semantic embeddings make that record navigable with natural
language. Keeping the two layers explicit and separable means each can be evaluated,
...
downstream reasoning step. CodeKG inverts this: the structural substrate is built
from a deterministic formal process, and the LLM is positioned where it is most
effective---synthesizing and explaining a context pack whose every line is verified
against the source. The result is a form of Structural KG-RAG that combines the
navigability of natural-language queries with the correctness guarantees of static
analysis.
